...
**Componentes BTP:**
- `object-store-file-manager-xsuaa` — XSUAA (autenticação/autorização, roles FileViewer/FileManager)
- `object-store-file-manager-objectstore` — Object Store service instance (s3-standard)
- `object-store-file-manager-html5repo` — HTML5 Application Repository (host do frontend)
- `object-store-file-manager-dest` — Destination Service (roteamento UI → backend)
...
2. Crie uma instância de **Object Store** com plano `s3-standard`:
   ```
   Nome: object-store-file-manager-objectstore
   ```
...
```bash
mbt build
# Gera: mta_archives/object-store-file-manager_1.0.0.mtar
```

### Deploy

```bash
cf deploy mta_archives/object-store-file-manager_1.0.0.mtar
```
...
2. Confirme que `com.sap.objectstorefilemanager` aparece na lista
3. No **SAP Build Work Zone**, crie um **Content Provider** apontando para o subaccount
4. No Launchpad, adicione o tile com:
   - App ID: `com.sap.objectstorefilemanager`
   - Intent: `ObjectStoreFileManager-manage`
...
| Blank page no Work Zone | `manifest.json` com ID errado | O `sap.app.id` deve ser `com.sap.objectstorefilemanager` |
